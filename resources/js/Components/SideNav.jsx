...
export default function SideNav({ mobileOpen = false, onClose = () => {} }) {
  const { url } = usePage();
  const isActive = (path) => (path === '/dashboard' ? url === '/' || url === '/dashboard' : url.startsWith(path));

  const navigation = (mobile = false) => (
    <>
      <div className="mb-5 p-4 flex items-start justify-between gap-3">
        <div><h1 className="text-3xl font-headline-md font-black text-[#1C1A27] tracking-tighter uppercase">VIRA</h1><p className="text-xs font-label-mono text-[#454654] mt-1 font-bold">VIRTUAL INCOME & RECORD ASSISTANT</p></div>
